...
class TikTokApp(App):
    def build(self):
        ...
        layout.add_widget(Label(text='TikTok Downloader', font_size='24sp', size_hint_y=0.2))
        
        self.url_input = TextInput(
            hint_text='Вставьте ссылку на TikTok...',
            multiline=False, size_hint_y=0.15, font_size='16sp'
        )
        layout.add_widget(self.url_input)
        
        self.btn = Button(
            text='СКАЧАТЬ В ГАЛЕРЕЮ', size_hint_y=0.15,
            background_color=(0.9, 0.2, 0.2, 1), font_size='18sp'
        )
        self.btn.bind(on_press=self.start_download)
        layout.add_widget(self.btn)
        
        self.status = Label(text='', font_size='14sp', color=(0.7, 0.7, 0.7, 1), size_hint_y=0.5)
